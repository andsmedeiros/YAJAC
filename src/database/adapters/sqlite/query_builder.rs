use crate::database::{
    ...
    query_parameters::{
        ...
        SearchParameters, SortDirection, SortParameters, SortingField,
    },
...
impl<'a> QueryBuilder<'a> {
    ...
    fn build_update_clause(
        ...
        Ok([
            ...
            [Attribute::Integer(id as i64)].as_slice(),
        ]
        .concat())
    }
    ...
    {
        if let Some(field) = attributes.find(|field| {
    ...
    fn parse_attribute<T: FromStr>(attribute: &str, value: &str) -> Result<T, Error> {
        value.parse::<T>().map_err(|_| Error::InvalidAttribute {
    ...
    fn parse_for_field(
        ...
        let attribute = match attribute_type {
            ...
            AttributeType::Integer => Attribute::Integer(Self::parse_attribute(field, value)?),
            AttributeType::Float => Attribute::Float(Self::parse_attribute(field, value)?),
            AttributeType::Boolean => Attribute::Boolean(Self::parse_attribute(field, value)?),
    ...
    fn fields_for_model(&self) -> impl Iterator<Item = &str> {
        slice::from_ref(&self.schema.primary_key.name)
            .into_iter()
            .chain(self.schema.foreign_keys.iter().map(|(name, _)| name))
            .chain(self.schema.attributes.iter().map(|(name, _)| name))
            .map(|name| *name)
    }
}
