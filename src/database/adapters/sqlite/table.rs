use crate::database::{
    ...
    attributes::{self, Attribute, Attributes, date_time_from_millis, date_time_from_rfc3339},
    error::Error,
...
mod tests {
    ...
    impl Context {
        pub(super) fn new() -> Self {
            ...
            connection.lock().unwrap().execute_batch(
                "\
        ...
        pub(super) fn table(&self) -> Table<'_> {
            Table { table_schema: &self.schema, connection: self.connection.clone() }
        }
    ...
    fn test_query() -> Result<(), Box<dyn StdError>> {
        ...
        assert_eq!(find_single_result[0].attributes.get("col1").unwrap(), &Attribute::Text("The quick brown fox".to_string()));
        ...
        assert_eq!(find_many_result[0].attributes.get("col3").unwrap(), &Attribute::Integer(42));
        assert_eq!(find_many_result[1].attributes.get("col3").unwrap(), &Attribute::Integer(1000));
        ...
        assert_eq!(text_search_result[0].attributes.get("col3").unwrap(), &Attribute::Integer(1000));
        assert_eq!(text_search_result[1].attributes.get("col3").unwrap(), &Attribute::Integer(-1000));
    ...
    fn test_first() -> Result<(), Box<dyn StdError>> {
        ...
        assert_eq!(result.unwrap().attributes.get("col1"), Some(&Attribute::Text("The quick brown fox".to_string())));
    ...
    fn test_insert() {
        ...
        assert_eq!(row.attributes.get("col1").unwrap(), &Attribute::Text("value1".to_string()));
    }

    #[test]
    fn test_update() {
        let context = Context::new();
        let insert_attributes = Attributes::from([
            ("col1".to_string(), Attribute::Text("value1".to_string())),
        ]);
        let insert_parameters = QueryParameters::default();
        context.table().insert(insert_attributes, &insert_parameters).unwrap();

        let update_attributes = Attributes::from([
        ...
        assert!(result.is_ok());
        let row = result.unwrap();
        assert_eq!(row.attributes.get("col1").unwrap(), &Attribute::Text("new_value".to_string()));
    }
